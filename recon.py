...
out_params_cnet = coarse_net(**input_params)
...
verts_gt, _ = params_to_hand(rh_model, inp_params, save=True, out_dir=os.path.join(cfg['out_dir'], 'recon'), fn_prefix=fn_prefixes, fn_suffix="_gt", color='green')
save_3d_object(verts=out_hand_verts_cnet.detach().cpu().numpy(), faces=rh_model.faces, out_dir=os.path.join(cfg.out_dir, 'recon'), fn_prefix=fn_prefixes, fn_suffix="_cnet", color='cyan')
save_3d_object(verts=out_hand_verts_rnet.detach().cpu().numpy(), faces=rh_model.faces, out_dir=os.path.join(cfg.out_dir, 'recon'), fn_prefix=fn_prefixes, fn_suffix="_rnet", color='navy')
...
save_3d_object(verts=transformed_verts, faces=transformed_faces, out_dir=os.path.join(cfg['out_dir'], 'recon'), fn_prefix=fn_prefixes, fn_suffix="_obj_mesh", color='yellow')
...
for loop_idx, file_idx in enumerate(idxs):
    ...
    np.savez_compressed(os.path.join(cfg['out_dir'], 'recon', f'{file_idx}_cnet_params.npz'), **cnet_dict)
    np.savez_compressed(os.path.join(cfg['out_dir'], 'recon', f'{file_idx}_rnet_params.npz'), **rnet_dict)
